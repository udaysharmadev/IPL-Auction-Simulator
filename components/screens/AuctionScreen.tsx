...
import { Bell, ChevronDown, CircleHelp, FastForward, Gavel, Search, Settings2, Shield, Sparkles, Volume2, VolumeX, X, ArrowRight, Check, ChevronLeft, LockKeyhole, Play, SlidersHorizontal, Target, Zap, MapPin, Users, Crown, AlertTriangle, BrainCircuit, Activity, History, Gauge, Eye, Flame, Keyboard } from "lucide-react";
    ...
    <section className="room-grid">
      ...
      <section className="auction-stage-wrap" aria-label="Auction stage">
        ...
        <div className={`auction-scene shot-${cameraShot.toLowerCase()}`}>
          <AuctionRoomCanvas tension={auction.tension} shot={cameraShot} accent={team.color} active={auction.phase !== "COMPLETE"} soundOn={auction.soundOn} quality={auction.graphicsQuality} />
          ...
          <div className="stage-platform"><div className="stage-screen"><div className="screen-kicker">NOW ON THE BLOCK <span>•</span> {auction.category}</div><div className="stage-player-portrait"><PlayerPortrait player={current} size={74} /></div><div className="screen-name">{current.identity.name.toUpperCase()}</div><div className="screen-role">{roleLabel(current.role.primary)} <span>•</span> {current.identity.nationality} <span>•</span> {current.auctionData.cappedStatus}</div><div className="screen-price">{auction.currentBid > 0 ? formatCr(auction.currentBid) : `Base ${formatCr(current.auctionData.basePrice ?? 1)}`}</div></div><div className="podium"><div className="podium-top"><Gavel size={19} /><span>AUCTIONEER</span></div><div className="podium-face" /></div></div>
